...
There is a ready-to-use sample trade list in `examples/sample_trade_list.csv`, so you can try the engine immediately:

```bash
# 30-second smoke test on the bundled sample data
python scripts/run_simulation.py --trades examples/sample_trade_list.csv --mode explore --n_per_cell 200 --jobs 4
```

With your own trades:

```bash
# Quick exploration (128 cells x 20K perms)
python scripts/run_simulation.py --trades your_trades.csv --mode explore --jobs 8

# Focused analysis (64 cells x 100K perms)
python scripts/run_simulation.py --trades your_trades.csv --mode focus --jobs 8

# Full publication run (6048 cells x 200K perms, many hours)
python scripts/run_simulation.py --trades your_trades.csv --mode full --jobs 8

# Or custom permutation count on the full grid
python scripts/run_simulation.py --trades your_trades.csv --n_per_cell 50000 --jobs 8
```

The `--trades` flag accepts any CSV path — the file does not need to be named `trade_list.csv`.

### Check Progress While Running

```bash
# Check the heartbeat file (replace run name with yours)
cat montecarlo_output/mc_run_YYYYMMDD_HHMMSS/aggregated/heartbeat.json

# Or use the status flag (no --trades needed)
python scripts/run_simulation.py --status mc_run_YYYYMMDD_HHMMSS
```

### Resume an Interrupted Run

```bash
# Ctrl+C anytime, then resume by run name (trades path is read from the run manifest)
python scripts/run_simulation.py --resume mc_run_YYYYMMDD_HHMMSS
```

### Analyze a Completed Run

```bash
python scripts/analyze_run.py --run_dir montecarlo_output/mc_run_YYYYMMDD_HHMMSS --export_csv
```

### Dry Run (Preview Config)

```bash
python scripts/run_simulation.py --trades your_trades.csv --mode explore --dry_run
```
